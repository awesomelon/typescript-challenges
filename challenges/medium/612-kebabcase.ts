...
type IsNotEmpty<T extends unknown[]> = T["length"] extends 0 ? false : true;

type ToLowerCase<S extends string, L extends boolean = false> =
...
> = S extends `${infer A}${infer B}`
  ? `${ToLowerCase<A, IsNotEmpty<Temp>>}${KebabCase<B, [...Temp, A]>}`
  : S;
...
type SS<S extends string> = S extends Lowercase<S> & Uppercase<S>
  ? false
  : true;
